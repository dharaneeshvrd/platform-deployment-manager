...
def check_in_service_log(application, component_name):
    '''
    Check in service log in case of application failed to submit to YARN
    '''
    service_name = '%s-%s-%s' % (CONFIG['environment']['namespace'], application, component_name)
    message = ''
    distro = platform.dist()
    usesSystemd = distro[0] in ('redhat', 'centos')
    if usesSystemd:
        out = commands.getoutput('sudo journalctl -u %s.service -n 100' % (service_name)).split('\n')
    else:
        out = commands.getoutput('sudo tail -n 100 /var/log/upstart/%s.log' % (service_name)).split('\n')
    for line in out:
        if 'Exception' in line:
            message = line.split('Exception:')[-1].strip()
            break
    ...
    if status == 'CREATED':
        ...
        if yarn_data != None:
            if timestamp < yarn_data['startedTime']:
                ...
                aggregate_status, information = check_in_service_log(application, component_name)
        else:
            aggregate_status, information = check_in_service_log(application, component_name)
    ret = {
...
def flink_yarn_handler(yarn_data, application, component_name):
    '''
    ...
    if yarn_data['state'] == 'SUBMITTED' or yarn_data['state'] == 'ACCEPTED':
        ...
        if len(flink_job_id) < 1:
            trackingUrl = 'http://%s/#/completed-jobs' % (CONFIG['environment']['flink'])
        else:
            trackingUrl = 'http://%s/#/jobs/%s' % (CONFIG['environment']['flink'], flink_job_id)    
...
def flink_application(job_name, application, component_name):
    ...
    if status == 'CREATED':
        if yarn_data != None:
            aggregate_status, yarnid, trackingUrl, information = flink_yarn_handler(yarn_data, application, component_name)
        else:
            aggregate_status = ApplicationState.CREATED
    else:
        if yarn_data != None:
            ...
            check_in_service = True
    if check_in_service:
        aggregate_status, message = check_in_service_log(application, component_name)
...
def find_split_count(applist):
    ...
    """
    (component_count, oozie_component_count, spark_component_count, flink_component_count) = (0, 0, 0, 0)
    split_count = 1
