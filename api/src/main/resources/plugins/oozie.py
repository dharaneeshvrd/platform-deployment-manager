...
class OozieCreator(Creator):
    ...
    def stage_flink_components(self, application_name, component_name, properties, staged_component_path):
        ...
        shutil.copyfile(os.path.join(this_dir, 'flink-cancel.py'), '%s/lib/flink-cancel.py' % staged_component_path)
        shutil.copyfile(os.path.join(this_dir, service_script), '%s/lib/%s' % (staged_component_path, service_script))
        self._fill_properties(os.path.join('%s/lib' % staged_component_path, "flink-cancel.py"), properties)
        ...
        deployer_utils.exec_ssh(target_host, root_user, key_file, mkdir_commands)
        ...
        deployer_utils.exec_ssh(target_host, root_user, key_file, copy_commands)
    ...
    def destroy_component(self, application_name, create_data):
        ...
        # cancel flink job and delete component from local
        if "flink_staged_path" in create_data:
            destroy_commands = ["python %s/flink-cancel.py" % create_data["flink_staged_path"],
    ...
    def stop_component(self, application_name, create_data):
        ...
        # cancel flink job
        if "flink_staged_path" in create_data:
            stop_commands = ["python %s/flink-cancel.py" % create_data["flink_staged_path"]]
