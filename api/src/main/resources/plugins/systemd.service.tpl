...
ExecStartPre=/opt/${environment_namespace}/${component_application}/${component_name}/yarn-kill.py '${dynamic_component_job_name}'
ExecStopPost=/opt/${environment_namespace}/${component_application}/${component_name}/yarn-kill.py '${dynamic_component_job_name}'
Environment=SPARK_MAJOR_VERSION=${component_spark_version}
ExecStart=${environment_spark_submit} --driver-java-options "-Dlog4j.configuration=file:////opt/${environment_namespace}/${component_application}/${component_name}/log4j.properties" --class ${component_main_class} --name '${dynamic_component_job_name}' --master yarn-cluster --files log4j.properties ${component_spark_submit_args} ${component_main_jar}
